...
class Stratagem:
    ...
    description: str
    effect_fn: Callable[[GameState, UnitState | None, dict[str, Any]], GameState]
    condition_fn: Callable[[GameState, UnitState | None], bool] | None = None

    def is_available(self, state: GameState, phase: str, target: UnitState | None = None) -> bool:
...
class StratagemRegistry:
    ...
    def get(self, name: str) -> Stratagem | None:
    ...
    def execute(
        self, state: GameState, name: str, target: UnitState | None = None, **kwargs
    ) -> GameState:
...
def _init_core_stratagems():
    ...
    def command_re_roll_effect(state: GameState, target: UnitState | None, **kwargs) -> GameState:
    ...
    def insane_bravery_effect(state: GameState, target: UnitState | None, **kwargs) -> GameState:
    ...
    def counter_offensive_effect(state: GameState, target: UnitState | None, **kwargs) -> GameState:
    ...
    def tank_shock_effect(state: GameState, target: UnitState | None, **kwargs) -> GameState:
        if target is not None:
