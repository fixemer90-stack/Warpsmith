...
## Task 6.1 checkpoint evidence — 2026-05-19

Task 6.1 fixed authoritative final snapshot/result consistency for CR-14, CR-18, and CR-24 scope. `/api/results/{game_id}` now derives `summary.final_victory_points` and `summary.winner` from authoritative `final_state.victory_points`, preventing stale summary metadata from driving `/result/{game_id}`. Focused suite: `66 passed`; full suite: `630 passed, 3 skipped`; Ruff/format/diff-check clean for Task 6.1 touched files.

## Task 6.2 checkpoint evidence — 2026-05-19

Task 6.2 was re-checked and two remaining CR-14/CR-18/CR-24 gaps were fixed before approval: actual VP logs with `(total: N)` / Battle Ready wording now parse as structured `vp` events, and result charge cards now use runtime-ID owner lookup via `chargeCount(0/1)` instead of `actor_id.startsWith('player2')`. Focused replay/result suite: `61 passed`; full suite: `633 passed, 3 skipped`; `node -c web/static/result_chart.js`, JS owner-lookup probe, Ruff check, Ruff format, and targeted `git diff --check` are green.
