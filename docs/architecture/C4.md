...
Каждая AI-симуляция получает уникальный идентификатор `auto_{seed}`, где `seed` — случайное целое 1–99999. Гарантирует неперезапись реплеев (ранее все бои использовали `auto_42`).

**Генерация seed — трёхуровневый фоллбэк:**

1. **Browser** (`scenario_setup.js`) — `Math.floor(Math.random() * 100000)`, передаётся в `POST /api/auto-play?seed=...`
2. **API** (`api.py:728`) — если параметр `seed` отсутствует: `random.randint(1, 99999)`
3. **Engine** (`autoplay.py:298`) — если `config.seed is None`: `np.random.default_rng().integers(1, 99999)`

**Хранение:** `game_id TEXT PRIMARY KEY` в таблице `replays`. Детерминирован при ручной передаче seed.
...
sequenceDiagram
    ...
    participant ENG as autoplay
    participant DB as SQLite

    JS->>API: POST /api/auto-play?seed=RANDOM
    Note right of JS: Math.random() * 100000

    alt seed missing
        API->>API: random.randint(1, 99999)
    end

    API->>ENG: run_auto_game(roster_a, roster_b, seed)

    alt seed is None
        ENG->>ENG: actual_seed = random
    end

    ENG->>DB: INSERT INTO replays
    DB-->>ENG: ok
    ENG-->>API: result: game_id
    API-->>JS: redirect: /round-viewer/auto_XXXXX
```
