...
### Added
- **Code Review Plan requirement** — добавлен `docs/requirements/code-review-plan.md`: план полной проверки всего кода, декомпозированный на 25 atomic review-задач (`CR-00`–`CR-24`). План основан на Obsidian-требовании `agent-skills-0.6.0/agents/code-reviewer` и фиксирует mandatory review framework по 5 измерениям: Correctness, Readability, Architecture, Security, Performance. Документ задаёт формат отчётов `docs/reviews/YYYY-MM-DD/*.md`, triage rules для Critical/Important/Suggestion, parallelization batches и final regression gate.
