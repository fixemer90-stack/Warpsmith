---
confidence: high
created: 2026-05-01
edition: 10e
faction: adeptus-mechanicus
icon: infantry
points: 50
role: other
slug: servitor-battleclade
sources:
- Faction Pack v1.1 (April 2026)
subtype: unit
tags:
- adeptus-mechanicus
- infantry
- cult-mechanicus
- servitor
- battleclade
squad_size: {'min': 2, 'max': 4, 'step': 1}
title: Servitor Battleclade
type: entity
updated: 2026-05-01
---

# Servitor Battleclade

> *"Servitor Battleclades provide their Adeptus Mechanicus masters with expendable and extremely versatile combat assets in the field."*

Often sent to recover vital archeotech or data, their mix of firepower, close quarters punch, and array of servo-tools allows their Underseers to achieve whatever task is assigned to them.

## Profiles

| Type | M | T | SV | W | LD | OC | INV |
|------|---|---|----|---|----|---|-----|
| Combat Servitors & Gun Servitors | 6" | 4 | 4+ | 2 | 7+ | 1 | 6+ |
| Servitor Underseer | 6" | 4 | 4+ | 2 | 7+ | 1 | 6+ |

## Unit Composition

- 1 Servitor Underseer
- 2 Gun Servitors
- 6 Combat Servitors

The **Servitor Underseer** is equipped with: Mechanicus pistol; dataspikes.

1 **Gun Servitor** is equipped with: heavy arc rifle; servo-claw.
1 **Gun Servitor** is equipped with: heavy bolter; servo-claw.

Every **Combat Servitor** is equipped with: phosphor blaster; servo-claw.

## Keywords

**FACTION KEYWORDS:** Adeptus Mechanicus

**KEYWORDS — ALL MODELS:** Infantry, Imperium, Cult Mechanicus, Servitor Battleclade

**UNDERSEER:** Tech-Priest

## Attached Unit

If a Character unit from your army with the Leader ability can be attached to a Kataphron Breachers unit, it can be attached to this unit instead.

## Abilities

### Faction: Doctrina Imperatives
This unit benefits from the Doctrina Imperatives army rule.

### Network Override
While this unit contains one or more Tech-Priest models, this unit is:

- Eligible to perform an Action in a turn in which it Advanced.
- Eligible to shoot in a turn in which it started an Action.

## Ranged Weapons

| Weapon | Range | A | BS | S | AP | D | Abilities |
|--------|-------|---|----|---|----|---|-----------|
| Heavy arc rifle | 30" | 1 | 4+ | 8 | -2 | 3 | [ANTI-VEHICLE 4+, RAPID FIRE 1] |
| Heavy bolter | 36" | 3 | 4+ | 5 | -1 | 2 | [HEAVY, SUSTAINED HITS 1] |
| Incendine igniter | 12" | D6 | N/A | 3 | 0 | 1 | [IGNORES COVER, TORRENT] |
| Mechanicus pistol | 12" | 1 | 4+ | 6 | -1 | 1 | [DEVASTATING WOUNDS, PISTOL] |
| Meltagun | 12" | 1 | 4+ | 9 | -4 | D6 | [MELTA 2] |
| Phosphor blaster | 24" | 1 | 4+ | 4 | 0 | 1 | [IGNORES COVER, RAPID FIRE 1] |

## Melee Weapons

| Weapon | Range | A | WS | S | AP | D |
|--------|-------|---|----|---|----|---|
| Dataspikes (Underseer) | Melee | 4 | 4+ | 4 | 0 | 1 |
| Servo-claw | Melee | 2 | 4+ | 4 | 0 | 1 |

## Wargear Options

- 1 Combat Servitor model can have its 1 phosphor blaster replaced with 1 meltagun.
- Up to 3 Combat Servitor models can each have their 1 phosphor blaster replaced with 1 incendine igniter.

## See Also

- [[Adeptus Mechanicus]] — faction overview