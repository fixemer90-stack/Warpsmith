...
async def auto_play_simulation(
    ...
    try:
        ...
        # Convert deployment string to DeploymentType enum
        try:
...
async def get_replay(game_id: str):
    ...
    from backend.engine.replay import load_replay
...
async def get_result(game_id: str):
    ...
    from backend.engine.replay import load_replay as _load_replay

    replay = _load_replay(db.conn, game_id)
    if replay is None:
        raise _HTTPException(status_code=404, detail=f"Result {game_id} not found")
    return _json.loads(_json.dumps(_asdict(replay), default=str))
