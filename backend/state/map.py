...
class BattlefieldMap:
    ...
    def __post_init__(self):
        ...
        if self.terrain.shape != (self.height, self.width):
            raise ValueError(f"Terrain array shape {self.terrain.shape} does not match map dimensions {self.height}x{self.width}")

    @classmethod
    def create_empty(cls, width: int = 6, height: int = 4, name: str = "Empty Battlefield") -> 'BattlefieldMap':
    ...
    def add_deployment_zone(self, name: str, deployment_type: DeploymentType,
                           coordinates: List[Tuple[int, int]], description: str = ""):
    ...
    def get_deployment_zone_for_player(self, player_id: str) -> Optional[DeploymentZone]:
