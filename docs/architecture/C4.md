...
CREATE TABLE replays (
    game_id     TEXT PRIMARY KEY,    -- "auto_{seed}" — уникальный seed на каждую симуляцию
...
## Game ID — правила генерации

Каждая AI-симуляция получает уникальный идентификатор:
...
| Уровень | Где | Приоритет |
|---------|-----|-----------|
| Frontend | `scenario_setup.js` → `Math.floor(Math.random() * 100000)` | 1 |
| API | `api.py` → если `seed is None` → `random.randint(1, 99999)` | 2 |
| Engine | `autoplay.py` → если `config.seed is None` → `np.random.default_rng().integers(1, 99999)` | 3 |

### Гарантии

- **Уникальность:** случайный seed 1-99999, шанс коллизии < 0.001% при 100 боях
- **Воспроизводимость:** если передать конкретный seed в API — результат детерминирован
- **Хранение:** `game_id` — PRIMARY KEY в таблице `replays`, каждый бой — отдельная запись
- **Старые реплеи не перезаписываются** (было: все бои = `auto_42`, теперь каждый уникален)

### Flow

```mermaid
sequenceDiagram
    participant JS as scenario_setup.js
    participant API as api.py (FastAPI)
    participant Engine as autoplay.py

    JS->>JS: seed = Math.random() * 100000
    JS->>API: POST /api/auto-play?seed=84721

    alt seed передан
        API->>API: seed = 84721
    else seed не передан
        API->>API: seed = random.randint(1, 99999)
    end

    API->>Engine: run_auto_game(config.seed)

    alt config.seed задан
        Engine->>Engine: actual_seed = config.seed
    else config.seed is None
        Engine->>Engine: actual_seed = np.random.integers(1, 99999)
    end

    Engine->>Engine: game_id = f"auto_{actual_seed}"
    Engine->>DB: save_replay(game_id, ...)
    Engine-->>API: result.game_id
    API-->>JS: { "game_id": "auto_84721" }
    JS->>Browser: redirect → /round-viewer/auto_84721
```
